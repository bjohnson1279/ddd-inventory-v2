...
export class ComplianceLedgerService {
  private static getPrivateKey(): string {
    ...
    if (!key) {
      console.warn("WARNING: COMPLIANCE_PRIVATE_KEY environment variable is not set. Using insecure development default private key. Please configure COMPLIANCE_PRIVATE_KEY in production.");
      return "dummy_compliance_private_key_please_change_in_production";
    }
    return key;
  }
